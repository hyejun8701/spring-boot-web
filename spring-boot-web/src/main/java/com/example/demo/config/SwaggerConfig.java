 ...
 * 
 * @Bean public UiConfiguration uiConfig() { return UiConfiguration.DEFAULT; }
 * 
 * private ApiInfo metadata() { return new ApiInfoBuilder()
 * .title("Spring Boot") .description("Spring Boot Rest API") .version("1.0")
 * .build(); }
 * 
 * @Bean public Docket api() { return new Docket(DocumentationType.SWAGGER_2)
 * .groupName("basic") .select() .apis(RequestHandlerSelectors.any())
